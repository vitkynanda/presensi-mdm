...
import { getAllPresenceData, getAllTeamData, getAllUserData } from "services";
import { useUiStateStore } from "store/ui-state";
import { columns } from "./data/presenceDataTable";

const PresenceTable = ({ onSetDate }) => {
  const {
    globalLoading,
    setGlobalLoading,
    allPresenceData,
  ...
  const handleFilter = async () => {
    ...
    const res = await getAllPresenceData(filter);
    if (res) {
      setAllPresenceData(res);
  ...
  useEffect(() => {
    (async () => {
      setGlobalLoading(true);
      const res = await getAllPresenceData();
      if (res) setAllPresenceData(res);
      setGlobalLoading(false);
    })();
  }, [setAllPresenceData, setGlobalLoading]);
  ...
  return (
    <Card>
      <SoftBox
        ...
        pt={3}
      >
        <SoftTypography variant="h5">Presences Data</SoftTypography>
        {matchesMd ? (
          <>
      ...
      >
        <SoftDataTable
          table={{ columns, rows: allPresenceData || [] }}
